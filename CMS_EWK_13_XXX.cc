...
namespace Rivet {
  ...
  class CMS_EWK_13_XXX:public Analysis {
	...
	if (fs.particles().at(i).pdgId() == PHOTON)
	  photons.push_back(i);
	if (fabs(fs.particles().at(i).pdgId()) == 12 || fabs(fs.particles().at(i).pdgId()) == 14 || fabs(fs.particles().at(i).pdgId()) == 16)
...
*/
	...
	if (Id == ELECTRON && fabs(eta) < 2.4) {
	  ...
	  for (unsigned int j = 0; j < photons.size(); j++) {
	    ...
	    if (delta_phi > acos(-1)) delta_phi = 2 * acos(-1) - delta_phi;
	...
	for (unsigned int i = 0; i < antimuon_dres.lepton_photon.size(); i++) {
	  canc_part.push_back(antimuon_dres.lepton_photon.at(i));
        }
      }
...
cout << "Nb = " << Nb << endl;
	...
	w_bjetmultiplicity->fill(vect_bjets.size(), weight);
